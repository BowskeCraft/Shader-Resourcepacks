...
vec3 depth_to_pos(vec2 coord,float depth, vec2 sampler_size){
...
vec3 pos_to_depth(vec3 pos, vec2 sampler_size){

    vec2 coord = pos.xy;
    coord /= (vec2(sampler_size.x / sampler_size.y,1.0) * 1.4 * pos.z);
    coord += 0.5;
...
vec3 depth_buffer_to_pos(sampler2D depth_sampler,vec2 sampler_size,vec2 coord){
    ...
    return depth_to_pos(coord,depth,sampler_size);
}

vec3 depth_to_normal(sampler2D depth_sampler,vec2 sampler_size,vec2 uv_pos){
