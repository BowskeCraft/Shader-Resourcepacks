...
float perlin_noise_t(vec3 p,float freqency,float phase){
    ...
    vec4 v     = corners[pcg_hash(vec4( p3.xyz,     loop( p3.w     , daylength) )) % 16u];
    vec4 vx    = corners[pcg_hash(vec4( p3x.xyz,    loop( p3x.w    , daylength) )) % 16u];
    vec4 vy    = corners[pcg_hash(vec4( p3y.xyz,    loop( p3y.w    , daylength) )) % 16u];
    vec4 vxy   = corners[pcg_hash(vec4( p3xy.xyz,   loop( p3xy.w   , daylength) )) % 16u];
    vec4 vz    = corners[pcg_hash(vec4( p3z.xyz,    loop( p3z.w    , daylength) )) % 16u];
    vec4 vxz   = corners[pcg_hash(vec4( p3xz.xyz,   loop( p3xz.w   , daylength) )) % 16u];
    vec4 vyz   = corners[pcg_hash(vec4( p3yz.xyz,   loop( p3yz.w   , daylength) )) % 16u];
    vec4 vxyz  = corners[pcg_hash(vec4( p3xyz.xyz,  loop( p3xyz.w  , daylength) )) % 16u];
    vec4 vw    = corners[pcg_hash(vec4( p3w.xyz,    loop( p3w.w    , daylength) )) % 16u];
    vec4 vxw   = corners[pcg_hash(vec4( p3xw.xyz,   loop( p3xw.w   , daylength) )) % 16u];
    vec4 vyw   = corners[pcg_hash(vec4( p3yw.xyz,   loop( p3yw.w   , daylength) )) % 16u];
    vec4 vxyw  = corners[pcg_hash(vec4( p3xyw.xyz,  loop( p3xyw.w  , daylength) )) % 16u];
    vec4 vzw   = corners[pcg_hash(vec4( p3zw.xyz,   loop( p3zw.w   , daylength) )) % 16u];
    vec4 vxzw  = corners[pcg_hash(vec4( p3xzw.xyz,  loop( p3xzw.w  , daylength) )) % 16u];
    vec4 vyzw  = corners[pcg_hash(vec4( p3yzw.xyz,  loop( p3yzw.w  , daylength) )) % 16u];
    vec4 vxyzw = corners[pcg_hash(vec4( p3xyzw.xyz, loop( p3xyzw.w , daylength) )) % 16u];
