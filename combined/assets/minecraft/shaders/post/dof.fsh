...
void main(){
    ...
    float focal_depth = texture(InDepthSampler, vec2(0.5)).r;//clamp(texture(InDepthSampler, vec2(0.5)).r,FocusMin,FocusMax);
    ...
    float depth_diff = (1.0 - 1.0 / (1.0 + abs(focal_depth - current_depth))) * DoFStrength;
